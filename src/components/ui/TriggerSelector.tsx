...
const TriggerSelector: React.FC<TriggerSelectorProps> = ({
  ...
  const getAllTriggers = () => {
    const allTriggers: { name: string; category: string; subcategory?: string }[] = [];
    
    Object.entries(triggerLibrary).forEach(([category, content]) => {
      if (Array.isArray(content)) {
        content.forEach(trigger => {
          allTriggers.push({ name: trigger, category });
        });
      } else {
        Object.entries(content).forEach(([subcategory, triggers]) => {
          triggers.forEach(trigger => {
            allTriggers.push({ name: trigger, category, subcategory });
          });
        });
      }
    });
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case '攻撃用': return 'border-red-500/50 bg-red-500/10 text-red-400';
      case '防御用': return 'border-blue-500/50 bg-blue-500/10 text-blue-400';
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {selectedTriggers.map((trigger) => {
          const triggerInfo = getAllTriggers().find(t => t.name === trigger);
          return (
            <div
              key={trigger}
              className={`flex items-center space-x-2 px-3 py-2 rounded-lg border ${getCategoryColor(triggerInfo?.category || '')}`}
            >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-green-500/20 rounded-xl p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              {Object.keys(triggerLibrary).map(category => (
                <button
                  key={category}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {filteredTriggers.map(({ name, category, subcategory }) => {
